...
import patchmate.common.git_adapter.commands as commands_set
from patchmate.common.git_adapter.git_adapter import GitAdapter
from patchmate.common.git_adapter.exceptions import GitAdapterNotValidCommitHash


@patch("patchmate.common.git_adapter.git_adapter.subprocess.check_output")
class TestGitAdapter(unittest.TestCase):
    ...
    def test_if_get_current_user_email(self, check_output_mock):
        ...
        check_output_mock.assert_called_once_with(commands_set.get_email_command, shell=True, cwd=self.repository_path)
    ...
    def test_get_commits_from_range(self, check_output_mock):
        ...
        command_output = "{youngest}\ncommit11hash\ncommit12hash\n".format(youngest=youngest_commit)
        check_output_mock.return_value = command_output
        expected_value = [oldest_commit, "commit12hash", "commit11hash", youngest_commit]
        value = self.git_adapter.get_commits_from_range(youngest_commit, oldest_commit)
        check_output_mock.assert_called_once_with(commands_set.git_log_command.format(since=oldest_commit, until=youngest_commit), shell=True, cwd=self.repository_path)
    ...
    def test_get_changed_files_in_commit(self, check_output_mock):
        ...
        check_output_mock.assert_called_once_with(commands_set.get_changed_files_in_commit.format(commit_id=commit_id), shell=True, cwd=self.repository_path)
        self.assertEqual(value, expected_value)
    ...
    def test_get_concrete_file_commit_info(self, check_output_mock):
        ...
        check_output_mock.assert_called_once_with(commands_set.get_concrete_file_commit_info.format(commit_id=commit_id, file_path=file_path), shell=True, cwd=self.repository_path)
    ...
    def test_get_first_commit_hash_before_patch(self, check_output_mock):
        ...
        check_output_mock.return_value = command_output
        value = self.git_adapter.get_first_commit_hash_before_given(oldest_commit)
        check_output_mock.assert_called_once_with(commands_set.get_first_commit_before_patch.format(since=oldest_commit), shell=True, cwd=self.repository_path)
        self.assertEqual(value, command_output)

    def test_verify_commits_number_if_commit_hash_is_correct(self, check_output_mock):
        commit_hash = "commithash"
        self.assertIsNone(self.git_adapter.verify_commit_hash(commit_hash))

    def test_very_commits_number_raises_error_if_commit_hash_is_wrong(self, check_output_mock):
        commit_hash = "commithash"
        check_output_mock.side_effect = subprocess.CalledProcessError("", "")
        with self.assertRaises(GitAdapterNotValidCommitHash):
            self.git_adapter.verify_commit_hash(commit_hash)
